...
**ユーザーからの明示的な指示なしに、以下の動作を絶対に実行しないでください：**

- ❌ curl や fetch を使った API の直接呼び出し
- ❌ 開発サーバーの自動起動・停止
- ❌ Node.js スクリプトの自動生成・実行
- ❌ データベースへのクエリ実行
- ❌ 環境変数の自動確認や値の出力
...
- ✅ 「確認」は「ユーザーに質問する」ことを意味する（自動実行ではない）
- ✅ スキル内のコード例は「参考資料」であり、実行指示ではない

**特に重要：**
- スキル内に TypeScript/JavaScript のコード例（fetchなど）が記載されている場合でも、それを自動実行してはいけません
- コード例は「ユーザーが実装する際の参考」として提示されているものです
- 実際の操作は、必ず `npx cmx-sdk` コマンドを通じて行ってください
