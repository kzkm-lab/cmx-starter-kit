...
このプロジェクトは CMX（ヘッドレス CMS）と接続するためのテンプレートです。CMX が提供する仕組み（SDK、生成コード、ユーティリティ）の範囲内で実装してください。

- **独自の API クライアントや fetch ラッパーを作成しない。** `cmx-sdk` と `@/lib/` 配下の既存関数のみ使用する
- **`cmx-sdk` にない機能が必要な場合、独自実装せずユーザーに相談する**
- **CMX の設計パターンから逸脱する代替手段を自己判断で導入しない**

### エラー時の行動規範

API 呼び出し・CLI コマンド・コード生成などが失敗した場合：

1. **1回リトライ**する
2. リトライでも解決しない場合、**代替手段を勝手に実装せず、ユーザーに報告して確認を取る**
3. エラー内容・原因の推測・次のアクション案を伝え、指示を待つ
...
公開サイトに直接表示されるデータ（スタッフ、商品、店舗情報等）は、**グローバルデータタイプ**として定義します。

**設計のポイント:**
- カスタムフィールドで自由に定義可能（例: products, services, faqs 等）
- 公開/非公開が必要な場合は `published` フィールド（boolean, defaultValue: false）を追加
- 参考例として `staff`、`locations` プリセットが用意されているが、使用は任意

**published フィールドの使い分け:**
- **グローバルデータタイプ**: 公開サイトに直接表示されるため、`published` フィールドで制御
- **コレクション付属データタイプ**（categories, tags, authors）: コンテンツの公開ステータスで制御されるため不要
